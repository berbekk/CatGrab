...
struct PieSegmentView: View {
    /// Какую часть сектора рисовать. Оверлей меню кладёт стекло всех секторов в один
    /// `GlassEffectContainer`, а обводку, иконку, цифру и лапку — отдельным слоем поверх:
    /// общий слой стекла перекрывает соседние по стеку view, и иконки внутри контейнера пропадают.
    enum Layer {
        case all
        case glass
        case content
    ...
    private var drawsGlass: Bool { layer != .content }
    private var drawsContent: Bool { layer != .glass }
    ...
    private var isUnassignedAction: Bool {
        appearsDisabled ?? (item.action == .unassigned)
    }
    ...
    @ViewBuilder
    private func sectorContent(center: CGPoint) -> some View {
        Group {
