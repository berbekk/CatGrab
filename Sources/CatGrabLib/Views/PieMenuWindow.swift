...
final class PieMenuWindowController {
    ...
    func show(
        ...
        let items = appCommands.map {
            AppCommandsMenuItems.build(actions: $0.actions, bundleIdentifier: $0.bundleIdentifier)
        ...
        let hapticFeedbackEnabled = ConfigManager.shared.configuration.hapticFeedbackEnabled
        ...
        let menuView = PieMenuView(
            ...
            onCommandSelected: { [weak self] action in
                self?.onCommandSelected?(action)
            },
    ...
    /// Клавиши при открытом меню: Esc, быстрый выбор и повтор хоткея.
    ...
    /// Быстрый выбор: пункт обычного меню или команда в меню команд. Недоступная команда ничего не делает.
    ...
    func hide() {
        ...
        // и меню дёргалось при отпускании.
