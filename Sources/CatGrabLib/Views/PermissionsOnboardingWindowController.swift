...
final class PermissionsOnboardingWindowController: NSObject, NSWindowDelegate {
    ...
    func present(onFinished: @escaping () -> Void) {
        ...
        // чтобы CatGrab уже стоял в списке «Универсальный доступ» к моменту, когда пользователь его откроет.
    ...
    /// Доступ выдан в Системных настройках — окно закрывается само и возвращает пользователя в CatGrab.
