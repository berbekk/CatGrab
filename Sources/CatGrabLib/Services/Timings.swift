...
enum Timings {
    ...
    /// Пауза перед повторным подключением к трекпадам после сна или подключения Magic Trackpad:
    /// устройство появляется в IOKit раньше, чем MultitouchSupport начинает отдавать кадры.
    static let trackpadDeviceRestartDelay: TimeInterval = 1.0
}
