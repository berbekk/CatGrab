...
/// и для `HotkeyRecorderView`, и для `KeystrokeRecorderView`.
        ...
        content
            ...
            .background(
                RoundedRectangle(cornerRadius: DS.Radius.m, style: .continuous)
                    .fill(
                        isRecording
                            ? DS.Colors.field.opacity(isHovered ? 1.0 : 0.95)
                            : DS.Colors.hotkeyBadgeFill.opacity(isHovered ? 1.0 : 0.96)
                    )
            )
            .overlay(
                RoundedRectangle(cornerRadius: DS.Radius.m, style: .continuous)
                    .strokeBorder(
                        isRecording
                            ...
                            : isHovered
                                ? DS.Colors.blueAccent.opacity(0.55)
                                : DS.Colors.hotkeyBadgeStroke,
                        lineWidth: isRecording ? DS.Border.focus : DS.Border.hairline
                    )
            )
        ...
        RecorderFieldChrome(
            ...
            HStack(spacing: DS.Spacing.s) {
                Image(systemName: "textformat.123")
                    ...
                    .foregroundStyle(DS.Colors.blueAccent.opacity(0.88))
        ...
        RecorderFieldChrome(isRecording: engine.isRecording, isHovered: $isHovered, onTap: toggle) {
            HStack(spacing: DS.Spacing.s) {
                Image(systemName: engine.isRecording ? "record.circle.fill" : "keyboard")
                    .font(DS.Typography.label)
                    .foregroundStyle(engine.isRecording ? DS.Colors.blueAccent : DS.Colors.blueAccent.opacity(0.88))
                    .accessibilityHidden(true)
                ...
                Text(engine.isRecording
                    ...
                    .foregroundStyle(engine.isRecording ? DS.Colors.blueAccent : DS.Colors.hotkeyLabel)
                    .lineLimit(1)
                ...
                if !engine.isRecording {
                    ...
                    .help(localizer.text(.hotkeyPickerMenuHelp))
                }
            }
        }
