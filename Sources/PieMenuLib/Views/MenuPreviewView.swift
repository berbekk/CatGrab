...
struct MenuPreviewView: View {
    ...
    var applySharedStyleActions: (applyToAll: () -> Void, chooseTargets: () -> Void)? = nil
    ...
    private var wheelCanvas: some View {
        GeometryReader { geo in
            ...
            let itemsToShow = draggedItemId != nil
                ...
                : previewItems
            let isGhost = menu.isRunningAppsMenu ? liveRunningAppsItems.isEmpty : menu.items.isEmpty
                ...
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        ...
                        if draggedItemId == nil && moved > 6 {
                            ...
                            guard !menu.isRunningAppsMenu, !menu.items.isEmpty else { return }
                            ...
                            guard let idx = absoluteSectorIndex(from: startA, radiusAtPoint: dist), idx < realItems.count else { return }
                            draggedItemId = realItems[idx].id
                        ...
                        if draggedItemId == nil && moved <= 6 {
                            ...
                            if dist < drawInnerRadius {
                                if let onAdd = onAddItem {
                                    onAdd()
                                } else if !menu.items.isEmpty || menu.isRunningAppsMenu {
                                    selectedItemId = nil
                                }
                            } else {
                                if menu.isRunningAppsMenu {
                            ...
                            if let fromPos = sorted.firstIndex(where: { $0.id == id }),
                               fromPos != target,
                               target < sorted.count {
    ...
    private var standardMenuShowsGhostPreview: Bool {
        !menu.isRunningAppsMenu && menu.items.isEmpty
    }

    private var canPasteMenuItem: Bool {
        guard !menu.isRunningAppsMenu, PieMenuItemPasteboard.read() != nil else { return false }
    ...
    private func pasteMenuItemFromClipboard() {
        guard !menu.isRunningAppsMenu, let template = PieMenuItemPasteboard.read() else { return }
        if !standardMenuShowsGhostPreview,
           let idx = hoveredSectorIndex,
           idx < realItems.count {
    ...
    private var applySharedStyleToolbarButton: some View {
        ...
        .help(localizer.text(.applySharedMenuSettings))
    }
}
